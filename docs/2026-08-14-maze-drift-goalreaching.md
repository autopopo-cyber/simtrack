...
## 十一、批量窄门统计（12 随机迷宫 × 单 0.8m 窄门）：门不窄，不用对门 ✅

**设计**：每个 seed 生成随机迷宫（随机 DFS 拓扑+墙抖动，路径 19-41 房间不等），**BFS 必经路径
中段一扇门改 0.8m**（其余 1.5m）。全真实栈（L2 10m±3cm + 5% 漂移 + 30s 自建图重定位 + A* 推进）。
600s/种子。基建：maze_gen `narrow_w` 参数 + meta 航点表/网格线/窄门标记；goal_runner 数据驱动
航点（修硬编码 seed42 债）；编排器 scripts/batch_run.py；分析 scripts/batch_analyze.py。

**结果**（11/12 种子有效，seed12 轨迹文件丢失仅存日志）：

| 指标 | 数值 |
|---|---|
| 到达终点 | 3/11（seed1/7/11，dist_goal≤1.4m）；路径>30房的长迷宫 600s 内走不完（正常） |
| **窄门遭遇/通过** | **6 遇 6 过，0 卡死**（5 个"未过"=没走到窄门位置，非失败） |
| **窄门房通过时间** | **median 23s = 普通房 median 23s**；mean 33 vs 30；p90 91 vs 41；max 115s（过，但摸索~1分钟） |
| 普通房通过时间 | median 23s，p90 41s，max 222s |
| 时间分布 | <30s: 72%房次/49%时间；>120s: 2%房次/**10%时间** |
| 慢点(>120s) | **3 个，全是普通房**，全部对应"步进失败4次跳过"（Nav2 连续 abort） |

**结论**：
1. **0.8m 窄门不构成瓶颈——不用做"对门"行为**。中位通过时间与普通门完全一致（23s），
   偶发的 91-115s 是对准角度多花时间，从不卡死。0.6m 全窄门迷宫的 4 试 4 败（§十）是
   极限工况；实际场景单 0.8m 门无感。
2. **真正的耗时大头是普通房的步进失败**（2% 房次吃 10% 时间）+ 个别 run 的定位损伤雪崩
   （seed8: 9 次扇形绕行 + slam_err 5.47m 复合恶化）。这是已知的 NavFn/门舞问题族，
   与门宽无关——下一步优化应打这里（步进失败的自动恢复/换路），而不是对门行为。
3. 方法论：**先统计后优化**——如果直接凭 §十 的 0.6m 单点结果写对门行为，就优化错了地方。

**复现**：`.venv/Scripts/python.exe scripts/batch_run.py`（131 分钟）→ `scripts/batch_analyze.py`。
数据在 results/batch1/。
