#!/usr/bin/env python3
"""萤火算法 Firefly v4 — find_gates+merge_gates + 黄球渲染

与V3区别: 门系统从 polygon_gates(多边形边缘中点→UNKNOWN门→A*死锁)
         改为 find_gates(栅格级前沿搜索→FREE门→A*可靠直达)
         新增黄球路点渲染 (每1m一个亮黄色球)
"""
...
MIX_THRESHOLD = 50
INIT_SCAN_STEPS = 200
LIDAR_TICK = 20; RENDER_SKIP = 100   # 5x实时
ARRIVE_THRESH = 1.0
...
print(f"━━━ 萤火 Firefly v4 ━━━ {VOXEL}m 三级跳A* 模式={EXPLORE_MODE} ━━━", flush=True)
...
if existing is not None:
    ...
    print(f"[NEW] 新扫图: mode={EXPLORE_MODE} v4")
    milestones = []
...
print(f"=== Firefly v4 start: fine={VOXEL}m jumps=1m/0.3m/0.1m gates={MAX_GATES} ===", flush=True)
