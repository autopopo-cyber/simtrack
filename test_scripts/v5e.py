#!/usr/bin/env python3
"""V5e: 门=开口两侧墙的最短连线中点。不在8米圆上，在两墙之间"""
import math, random, time, numpy as np, mujoco
from PIL import Image

LIDAR_RANGE=15.0; LIDAR_RAYS=120; SPEED=5.0; YAW_RATE=6.0
ARRIVE_THRESH=1.0; PLAN_INTERVAL=200
BOUNCE_FORCE_DURATION=0.3; STUCK_TIMEOUT=300; STUCK_DIST_THRESH=0.5
MAP="/home/qin/workspace/simtrack/confirmed/track_clean.png"
PIX_PER_M=40; FINISH=(3.0,95.0)
GATE_MIN_WIDTH=1.0  # 门最小宽度(米)

TRACK_ARR=np.array(Image.open(MAP))
def png_is_wall(wx,wy):
    px,py=int(wx*PIX_PER_M),int(wy*PIX_PER_M)
    if px<0 or px>=TRACK_ARR.shape[1] or py<0 or py>=TRACK_ARR.shape[0]: return True
    v=TRACK_ARR[py,px] if TRACK_ARR.ndim==2 else int(np.mean(TRACK_ARR[py,px,:3]))
    return v<50

def scan_rays(bx,by):
    """120线→每条返回(角度, 墙距, 墙x, 墙y), 无墙=8m"""
    rays=[]
    for a in np.linspace(0,2*math.pi,LIDAR_RAYS):
        cs,sn=math.cos(a),math.sin(a); hit=False
        for d in np.arange(0.1, LIDAR_RANGE+0.1,0.1):
            wx,wy=bx+cs*d,by+sn*d
            if png_is_wall(wx,wy): rays.append((a,d,wx,wy)); hit=True; break
        if not hit: rays.append((a,LIDAR_RANGE,bx+cs*LIDAR_RANGE,by+sn*LIDAR_RANGE))
    return rays

def build_gates(rays,bx,by):
    """找开口: 连续无墙弧→门=开口两侧墙点连线中点"""
    n=len(rays)
    # 标记每条射线是否撞墙
    is_wall=[r[1]<LIDAR_RANGE-0.05 for r in rays]

    # 找墙段和开口段
    gates=[]
    i=0
    while i<n:
        # 跳过墙段
        while i<n and is_wall[i]: i+=1
        if i>=n: break
        # 开口段开始
        start=i
        while i<n and not is_wall[i]: i+=1
        end=i-1
        if start>end: continue

        # 开口两侧的墙点
        left_idx=(start-1)%n  # 左侧墙的最后一条射线
        right_idx=(end+1)%n   # 右侧墙的第一条射线

        _,_,x1,y1=rays[left_idx]
        _,_,x2,y2=rays[right_idx]

        # 开口宽度（两侧墙点之间的距离）
        width=math.hypot(x2-x1,y2-y1)
        if width<GATE_MIN_WIDTH: continue

        # 门=两侧墙点连线中点
        mx,my=(x1+x2)/2,(y1+y2)/2
        gates.append((math.atan2(my-by,mx-bx),mx,my,width))

    return gates

def pick_gate_right(gates,yaw,bx,by):
    if not gates: return None
    yaw_n=yaw%(2*math.pi)
    cand=[((yaw_n-ga)%(2*math.pi),gx,gy,w) for ga,gx,gy,w in gates]
    cand.sort(key=lambda x:x[0])
    gx,gy=cand[0][1],cand[0][2]
    return gx,gy

class Mover:
    def __init__(self,m,d):
        self.m,self.d=m,d; self.yaw=math.pi/2; self.speed=SPEED; self.bounce=0
        self.force=0; self.escaping=False; self.stuck_t=0; self.stuck_x=0.0; self.stuck_y=0.0
    def step(self,tx,ty,step_no):
        bx,by=self.d.qpos[0],self.d.qpos[1]; dt=self.m.opt.timestep
        if not self.escaping:
            err=(math.atan2(ty-by,tx-bx)-self.yaw+math.pi)%(2*math.pi)-math.pi
            self.yaw+=max(-YAW_RATE*dt,min(YAW_RATE*dt,err)); self.speed=SPEED
        vx=math.cos(self.yaw)*self.speed; vy=math.sin(self.yaw)*self.speed
        if step_no-self.stuck_t>STUCK_TIMEOUT:
            if math.hypot(bx-self.stuck_x,by-self.stuck_y)<STUCK_DIST_THRESH: self._bounce(90,180)
            self.stuck_t=step_no; self.stuck_x=bx; self.stuck_y=by
        if self.force>0: self.force-=1; self.d.qvel[0]=vx; self.d.qvel[1]=vy
        elif self.escaping: self.escaping=False; self.d.qvel[0]=vx; self.d.qvel[1]=vy
        else: self.d.qvel[0]=vx; self.d.qvel[1]=vy
        mujoco.mj_step(self.m,self.d)
    def _bounce(self,lo,hi):
        if not self.escaping: self.bounce+=1; self.escaping=True
        self.yaw+=math.radians(random.uniform(lo,hi)*random.choice([-1,1])); self.d.qvel[:]=0
        self.force=int(BOUNCE_FORCE_DURATION/(SPEED*self.m.opt.timestep))

FIXED_SEED=random.randint(0,999999)
print(f"V5e seed={FIXED_SEED} 门=开口两侧墙最短连线",flush=True)

random.seed(FIXED_SEED); obs=[]
road=set()
for y in range(TRACK_ARR.shape[0]):
    for x in range(TRACK_ARR.shape[1]):
        v=TRACK_ARR[y,x] if TRACK_ARR.ndim==2 else int(np.mean(TRACK_ARR[y,x,:3]))
        if 50<=v<=200: road.add((x,y))
for _ in range(200):
    if len(obs)>=12 or not road: break
    px,py=random.sample(sorted(road),1)[0]; wx,wy=px/PIX_PER_M,py/PIX_PER_M
    if 2<wx<98 and 2<wy<98:
        if not any(math.hypot(wx-ox,wy-oy)<3.0 for ox,oy in obs):
            if math.hypot(wx-3,wy-3)>5 and math.hypot(wx-FINISH[0],wy-FINISH[1])>5: obs.append((wx,wy))
print(f"[OBS] {len(obs)}",flush=True)

OBS_XML="".join(f'<body name="o{i}" pos="{x:.1f} {y:.1f} 2.0"><geom type="cylinder" size="1.0 2.0" rgba="0.9 0.2 0.2 0.9"/></body>' for i,(x,y) in enumerate(obs))
xml=f'<mujoco><compiler angle="radian"/><option timestep="0.005"/><visual><global offwidth="1280" offheight="720"/></visual><asset><hfield name="t" size="50.0 50.0 4.0 2.0" file="{MAP}"/></asset><worldbody><light pos="50 50 80" dir="0 0 -1"/><body mocap="true" pos="3.0 95.0 2"><geom type="sphere" size="1.5" rgba="0.2 1.0 0.2 0.8"/></body>{OBS_XML}<geom type="hfield" hfield="t" pos="50 50 0.0" rgba="0.25 0.30 0.35 1.0" friction="0 0 0"/><body name="bot" pos="3.0 3.0 0.5"><joint type="slide" axis="1 0 0" damping="0"/><joint type="slide" axis="0 1 0" damping="0"/><geom type="cylinder" size="0.5 0.5" rgba="1 0.3 0 1" friction="0 0 0"/></body></worldbody></mujoco>'

m=mujoco.MjModel.from_xml_string(xml); d=mujoco.MjData(m); mv=Mover(m,d)
d.qpos[0]=3.0; d.qpos[1]=3.0
step=0; t0=time.time(); target_gate=None; MAX_STEPS=100000

print("START",flush=True)
while step<MAX_STEPS:
    bx,by=d.qpos[0],d.qpos[1]
    if math.hypot(bx-FINISH[0],by-FINISH[1])<3.0:
        print(f"\nARRIVED @({bx:.1f},{by:.1f}) step={step}",flush=True); break
    if step%PLAN_INTERVAL==0:
        rays=scan_rays(bx,by)
        gates=build_gates(rays,bx,by)
        gate=pick_gate_right(gates,mv.yaw,bx,by)
        if gate:
            target_gate=gate
            nw=sum(1 for r in rays if r[1]<LIDAR_RANGE-0.05)
            print(f"  [GATE] [{step}] ->({gate[0]:.1f},{gate[1]:.1f}) gates={len(gates)} wall_rays={nw}",flush=True)
        else:
            target_gate=FINISH
            print(f"  [NOGATE] [{step}] ->finish",flush=True)
    if target_gate:
        mv.step(target_gate[0],target_gate[1],step)
        if math.hypot(target_gate[0]-bx,target_gate[1]-by)<ARRIVE_THRESH: target_gate=None
    else:
        mv.step(FINISH[0],FINISH[1],step)
    step+=1
    if step%2000==0: print(f"  ... step={step} @({bx:.1f},{by:.1f})",flush=True)

print(f"done: step={step} t={time.time()-t0:.1f}s bounce={mv.bounce}",flush=True)
