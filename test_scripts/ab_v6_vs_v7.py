#!/usr/bin/env python3
"""对比V6连通性 vs V7递归多边形，同一批wall_points"""
...
# ====== V6 connectivity ======
def _point_key(p): return (int(p[0]/V6_VOXEL), int(p[1]/V6_VOXEL))
...
def _bridge_endpoints(all_endpoints, adj):
    ep_to_comp = {}
    for i,eps in enumerate(all_endpoints):
        for ep in eps: ep_to_comp[ep] = i
    candidates = []
...
# 跑
lines_v7, calls = polygon_boundary(nearby, bx, by)
...
all_lines = []; all_eps = []
for comp in comps:
    cl, eps = _trace_component(comp, adj)
    all_lines.extend(cl); all_eps.append(eps)
all_lines.extend(_bridge_endpoints(all_eps, adj))

blues_v6 = sum(1 for _,_,_,_,c in all_lines if c=='blue')
yellows_v6 = sum(1 for _,_,_,_,c in all_lines if c=='yellow')

print(f"V7 (递归多边形):  points={len(nearby)} poly顶点={blues_v7+yellows_v7} blues={blues_v7} yellows={yellows_v7} calls={calls}")
print(f"V6 (连通性):      points={len(nearby)} comps={len(comps)} lines={blues_v6+yellows_v6} blues={blues_v6} yellows={yellows_v6}")
"""
