...
def scan_walls(bx,by):
    ...
    for a in np.linspace(0,2*math.pi,LIDAR_RAYS):
        cs,sn=math.cos(a),math.sin(a); hit=False
        for d in np.arange(0.1, LIDAR_RANGE+0.1,0.1):
            wx,wy=bx+cs*d,by+sn*d
            if png_is_wall(wx,wy): hits.append((a,wx,wy,True)); hit=True; break
        if not hit: hits.append((a, bx+cs*LIDAR_RANGE, by+sn*LIDAR_RANGE, False))
    hits.sort(key=lambda h:h[0])
    walls=[]; i=0; n=len(hits)
    while i<n:
        if not hits[i][3]: i+=1; continue
        start=i
        while i<n and hits[i][3]: i+=1
        seg=hits[start:i]
        if len(seg)<2: continue
        walls.append((seg[0][0], seg[-1][0], sum(h[1] for h in seg)/len(seg), sum(h[2] for h in seg)/len(seg)))
    return walls

def pick_gate_right(walls,bx,by,yaw):
    if len(walls)<1: return None
    gates=[]; n=len(walls)
    for i in range(n):
        j=(i+1)%n
        x1,y1=bx+math.cos(walls[i][1])*LIDAR_RANGE, by+math.sin(walls[i][1])*LIDAR_RANGE
        x2,y2=bx+math.cos(walls[j][0])*LIDAR_RANGE, by+math.sin(walls[j][0])*LIDAR_RANGE
        mx,my=(x1+x2)/2,(y1+y2)/2
        ang=math.atan2(my-by,mx-bx)
        gates.append((ang,mx,my))
...
class Mover:
    def __init__(self,m,d):
        self.m,self.d=m,d; self.yaw=math.pi/2; self.speed=SPEED; self.bounce=0
        self.force=0; self.escaping=False; self.stuck_t=0; self.stuck_x=0.0; self.stuck_y=0.0
    def step(self,tx,ty,step_no):
        bx,by=self.d.qpos[0],self.d.qpos[1]; dt=self.m.opt.timestep
        if not self.escaping:
            err=(math.atan2(ty-by,tx-bx)-self.yaw+math.pi)%(2*math.pi)-math.pi
            self.yaw+=max(-YAW_RATE*dt,min(YAW_RATE*dt,err)); self.speed=SPEED
    ...
    def _bounce(self,lo,hi):
        ...
        self.yaw+=math.radians(random.uniform(lo,hi)*random.choice([-1,1])); self.d.qvel[:]=0
        self.force=int(BOUNCE_FORCE_DURATION/(SPEED*self.m.opt.timestep))

FIXED_SEED=random.randint(0,999999)
print(f"V5c seed={FIXED_SEED} 8m circle cutting", flush=True)

# obstacles
random.seed(FIXED_SEED); obs=[]
road=set()
...
for _ in range(200):
    ...
    px,py=random.sample(sorted(road),1)[0]; wx,wy=px/PIX_PER_M,py/PIX_PER_M
    if 2<wx<98 and 2<wy<98:
        if not any(math.hypot(wx-ox,wy-oy)<3.0 for ox,oy in obs):
            if math.hypot(wx-3,wy-3)>5 and math.hypot(wx-FINISH[0],wy-FINISH[1])>5: obs.append((wx,wy))
print(f"[OBS] {len(obs)}", flush=True)

OBS = "".join(f'<body name="o{i}" pos="{x:.1f} {y:.1f} 2.0"><geom type="cylinder" size="1.0 2.0" rgba="0.9 0.2 0.2 0.9"/></body>' for i,(x,y) in enumerate(obs))
xml = f'<mujoco><compiler angle="radian"/><option timestep="0.005"/><visual><global offwidth="1280" offheight="720"/></visual><asset><hfield name="t" size="50.0 50.0 4.0 2.0" file="{MAP}"/></asset><worldbody><light pos="50 50 80" dir="0 0 -1"/><body mocap="true" pos="3.0 95.0 2"><geom type="sphere" size="1.5" rgba="0.2 1.0 0.2 0.8"/></body>{OBS}<geom type="hfield" hfield="t" pos="50 50 0.0" rgba="0.25 0.30 0.35 1.0" friction="0 0 0"/><body name="bot" pos="3.0 3.0 0.5"><joint type="slide" axis="1 0 0" damping="0"/><joint type="slide" axis="0 1 0" damping="0"/><geom type="cylinder" size="0.5 0.5" rgba="1 0.3 0 1" friction="0 0 0"/></body></worldbody></mujoco>'

m=mujoco.MjModel.from_xml_string(xml); d=mujoco.MjData(m); mv=Mover(m,d)
d.qpos[0]=3.0; d.qpos[1]=3.0
step=0; t0=time.time(); target_gate=None; MAX_STEPS=100000
...
while step<MAX_STEPS:
    ...
    if step%PLAN_INTERVAL==0:
        ...
        gate=pick_gate_right(walls,bx,by,mv.yaw)
        if gate:
            target_gate=gate
    ...
    if target_gate:
        ...
        if math.hypot(target_gate[0]-bx,target_gate[1]-by)<ARRIVE_THRESH: target_gate=None
    else: mv.step(FINISH[0],FINISH[1],step)
    step+=1
